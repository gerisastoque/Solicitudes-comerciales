...
export function NodoProposalCard({ p }: Props) {
  return (
    <Link
    ...
    >
      ...
      <div className="mt-3 space-y-1 border-t border-border pt-2 text-xs text-muted-foreground">
        <div className="flex items-center gap-1.5">
          ...
          <span>Creada {format(new Date(p.createdAt), "d MMM yyyy", { locale: es })}</span>
          <span className="ml-auto font-medium text-foreground">{daysLabel(p.deadline)}</span>
        </div>
        {p.totalCost != null && (
