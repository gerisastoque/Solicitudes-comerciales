...
import { LdpProposalCard } from "@/components/LdpProposalCard";
import {
  MOCK_LDP_PROPOSALS,
  LDP_STATUS_META,
  LDP_COLUMNS,
...
export default function LdpBoard() {
  ...
  const filtered = useMemo(() => {
    return MOCK_LDP_PROPOSALS.filter((p) => {
      if (q && !`${p.title} ${p.applicant} ${p.id} ${p.company}`.toLowerCase().includes(q.toLowerCase())) return false;
  ...
  const grouped = useMemo(() => {
    const g: Record<LdpStatus, typeof MOCK_LDP_PROPOSALS> = {
      nuevo: [],
      proceso: [],
      costeo: [],
      aprobacion: [],
      ...
      <div>
        <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <h1 className="font-display text-2xl font-bold tracking-tight">
              {isColumnView ? LDP_STATUS_META[columnView as LdpStatus].label : "Propuestas"}
            </h1>
            <p className="mt-1 text-sm text-muted-foreground">
              {isColumnView ? `${columnItems.length} propuestas en esta columna` : `${filtered.length} propuestas encontradas`}
            </p>
          </div>
        </div>
            ...
            {!isColumnView && (
              <Select value={status} onValueChange={setStatus}>
                ...
                <SelectContent>
                  ...
                  {LDP_COLUMNS.map((c) => <SelectItem key={c} value={c}>{LDP_STATUS_META[c].label}</SelectItem>)}
                </SelectContent>
              </Select>
            )}
        ...
        {isColumnView && (
          <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {columnItems.map((p) => <LdpProposalCard key={p.id} p={p} />)}
                  ...
                  >
                    ...
                    <div className="flex flex-1 flex-col gap-2">
                      {items.length === 0 ? (
                        ...
                        visible.map((p) => <LdpProposalCard key={p.id} p={p} />)
                      )}
                      {overflow > 0 && (
                        <Link
                          to={`/ldp/propuestas?columna=${col}`}
